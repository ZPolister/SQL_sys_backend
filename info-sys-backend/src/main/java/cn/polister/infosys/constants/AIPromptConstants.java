...
public class AIPromptConstants {
    public static String ANALYSIS_PROMPT = """
        ...
        2. 疾病风险评估
        3. 提出一个目标建议（在减重（WEIGHT_LOSS）、运动热量（EXERCISE_CALORIES）、血糖控制（BLOOD_SUGAR）、血脂控制（BLOOD_LIPID）四个方面四选一，给出理由、目标数值、期望达成日期，先使用文字描述，再使用<goal>{json}</goal>按以下JSON格式嵌入在最后）：
        {
